...
- [x] 4.3 收集反馈并定稿模板：确认「文章内容大纲」的字段（slug/目标位置/章节骨架/关键数据点/配图计划/服务对象）是否够用、是否多余、"这一节写什么"的粒度是否合适；据此修模板与 `openspec-gate.md`，**改完才进入第 5 组**（2026-09 用户反馈：字段与粒度「目前可以」，模板定稿无改动）
