# digital-human-content-org Specification

## Purpose

定义博客中数字人主题内容的组织规范：所有数字人文章归位到 `categories/AI/数字人` 统一分类树，Hub 页（`digital-human-hub.html`）完整覆盖全部数字人文章并承载数字人扩展定义框架，与 `about-digital-human` 形成定义入口联动，且整理后的构建与导航（分类页、索引页、别名跳转、`sub_id` 排序）保持可用。
