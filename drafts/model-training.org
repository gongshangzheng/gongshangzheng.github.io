...
#+UPDATED_AT: 2026-07-12T00:56:16
#+PUBLISHED_AT: 
#+PUBLISHED_FILE: 
* 模型训练
#+caption: 标准完整训练流程
[[file:assets/model-training/standard-flow.webp]]
...
#+caption: 模型初始化流程
[[file:assets/model-training/initialization.webp]]

*** 前向传播
激活函数；CNN（卷积层 + 激活层 + 池化层轮流堆放，最简单的 CNN 就是这三层堆叠）；Transformer；还有 Dropout、归一化。

#+caption: 激活函数演示
[[file:assets/model-training/activation.webp]]

**** 归一化
#+caption: 归一化
[[file:assets/model-training/normalization.webp]]
...
[\mu/j = \frac{1}{B}\sum/{i=1}^{B} x_{i,j},\quad \sigma/j^2 = \frac{1}{B}\sum/{i=1}^{{B}(x_{i,j}-\mu_j)}2]

[\hat{x}_{i,j} = \gamma_j \cdot \frac{x_{i,j}-\mu_j}{\sqrt{\sigma_j^2+\epsilon}} + \beta_j]

*LayerNorm*------统计量下标是 *i（样本）*，沿 *特征维 H* 聚合：

[\mu/i = \frac{1}{H}\sum/{j=1}^{H} x_{i,j},\quad \sigma/i^2 = \frac{1}{H}\sum/{j=1}^{{H}(x_{i,j}-\mu_i)}2]

[\hat{x}_{i,j} = \gamma_j \cdot \frac{x_{i,j}-\mu_i}{\sqrt{\sigma_i^2+\epsilon}} + \beta_j]

*RMSNorm*------= LayerNorm 砍掉减均值和 β，只除以 RMS：

[\text{RMS}/i = \sqrt{\frac{1}{H}\sum_{j=1}^{H} x_{i,j}^2},\quad \hat{x}/{i,j} = \gamma_j \cdot \frac{x_{i,j}}{\text{RMS}_i}]
...
*** 损失函数
分两种： - *有监督 loss*（对比 ground truth）：CE loss、MSE loss、L1 loss。 - *无监督/无参考 loss*（不直接对比 GT，如对抗损失、感知损失）。
...
#+caption: 正则化对模型效果的影响
[[file:assets/model-training/regularization.webp]]

*** 反向传播
链式法则，逐层求梯度。没什么好说的。

*** 更新
优化器、学习率调度、权重衰减（weight decay）这三个。 - *学习率调度*：不同阶段给不同学习率，一般初期高（离目标远）、后期低。 - 优化器、权重衰减------见下方 TODO。
