...
## Local user constraint

For this blog/course-notes repository, do not generate SVG unless the user explicitly overrides this preference. The user has rejected generated SVG quality; prefer `docpage`/`docref`, Mermaid, JSXGraph, tables, formulas, or textual structure instead.

## Required Phases for Diagram Generation

Use these phases for every non-trivial SVG diagram, especially technical diagrams with arrows, signal paths, state transitions, circuit blocks, or multiple branches. Do not jump directly into drawing final SVG markup.

### Phase 1: Semantic Decomposition

List the diagram elements before writing SVG:

- **Nodes**: named objects such as adders, delays, gains, states, modules, labels.
- **Edges**: directed relationships, signal paths, feedback paths, or animation paths.
- **Groups**: logical regions such as input lane, output lane, feedback lane, control lane.
- **Priority**: identify the main reading path and secondary paths.

### Phase 2: Layout Plan

Choose an explicit layout strategy before writing coordinates:

- **Lane layout**: assign separate horizontal or vertical lanes for main path, feedforward paths, and feedback paths.
- **Grid layout**: place nodes on a coarse grid, usually with 80-140 units between adjacent nodes.
- **Orthogonal routing**: prefer `M x y L x y L x y` right-angle paths for technical diagrams.
- **Reserved channels**: keep separate y-levels or x-levels for return paths, feedback paths, and annotations.

For signal-flow diagrams, first decide whether the source material uses a **control-system block diagram** style or a **DSP signal-flow graph** style. If the user asks for 信号流图 / signal-flow graph in a DSP course context, default to the DSP textbook style:

| Element | DSP signal-flow graph convention |
|---------|----------------------------------|
| Nodes | Small solid dots on wires; do not use large circular `+` summing nodes unless the source explicitly does |
| Main signal | One thick horizontal top trunk from `x(n)` to `y(n)` |
| Delays | Vertical state chain with `z^{-1}` labels between state nodes |
| Feedback coefficients | Horizontal branches from delayed states back toward the left/input side, usually below the trunk |
| Feedforward coefficients | Horizontal branches from delayed states toward the right/output side |
| Cascade sections | Repeat the same compact cell left-to-right; each cell owns its own delay chain and coefficient branches |
| Parallel sections | Separate upper/lower branches that recombine at the output, with no crossing between branches |

Do **not** draw DSP signal-flow graphs as big module diagrams with large adders, rectangles, or long routed feedback loops. That style often creates visual clutter and is not what DSP lecture notes mean by 信号流图.

Use separate lanes by default:

| Lane | Purpose |
|------|---------|
| Main lane | input → processing → output |
| Feedforward lane | delayed input/state taps into output side |
| Feedback lane | delayed output/state taps back into input side |
| Annotation lane | labels, dashed guides, explanatory text |

### Phase 3: Collision and Overlap Check

Before finalizing SVG, check the layout against these rules:

1. **No edge through node**: a line must not pass through a rectangle, circle, triangle, text label, or arrow marker unless it terminates there.
2. **No hidden parallel edges**: two paths must not share the same segment unless they intentionally represent the same signal bus. If they share a segment, label it clearly or separate by at least 16 units.
3. **No label overlap**: labels must not overlap node borders, arrowheads, or other labels.
4. **Feedback paths get their own channel**: feedback loops should route around the main diagram, not cross the main signal path.
5. **Arrowhead clearance**: leave at least 10 units between arrowheads and nearby node borders or labels.
6. **Text clearance**: leave at least 6-8 units between text and surrounding shapes.
7. **Viewport margin**: keep at least 20 units of empty margin around the outermost stroke or label.

If any rule fails, change the layout; do not explain the issue in the caption as a workaround.

### Phase 4: Render and Accessibility

Write the final SVG only after the layout plan passes the collision check:

- Add `role="img"` and either `aria-label` or `<title>/<desc>`.
- Keep reusable markers, gradients, and filters inside `<defs>`.
- Use unique IDs for markers and animation targets to avoid collisions when multiple SVGs appear on one page.
- For animated SVG, add a `prefers-reduced-motion` fallback.
- Captions should describe the content, not the drawing instructions.

### Phase 5: Visual QA

After generating the SVG:

- Re-read the SVG source and inspect every path coordinate against the node coordinates.
- Confirm no path shares a long segment with another unrelated path.
- Confirm no path crosses text or node interiors.
- Confirm the diagram still reads in the intended order.
- If a build step exists, run it and inspect the generated page or exported image when possible.
