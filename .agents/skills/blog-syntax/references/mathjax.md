...
### ⚠️ 硬性禁止：display math 不得包裹在 `<p>` 内

**❌ 绝对错误**（会导致渲染异常、TOC 错位、样式丢失）：

```html
<p>$$s(t) = \sum_{n} a_n g(t - nT_s)$$</p>
<p>\[\mathcal{L} = \alpha T^2\]</p>
<p style="text-align:center">$$x^2$$</p>
```

**✅ 正确写法**（display math 独占一行，前后无 `<p>`）：
...
> **原因**：HTML 规范不允许 `<p>` 内嵌套 block 级元素（`<div>`）。构建器虽会尝试吸收 `<p>` 并提升属性，但在复杂段落结构下容易失败，导致 MathJax 渲染异常或后续内容泄漏。**所有新文章必须直接写裸 `$$...$$` / `\[...\]`，不要手动包 `<p>`。**
