...
引用使用 `#key#` 语法，构建时转换为 `<cite>#key</cite>`。
...
**与行内公式的区分：** 行内公式用 `$...$`，引用用 `#key#`。引用 key 建议只使用字母、数字、下划线、连字符、点号和冒号。
...
1. 正文使用 `#Author-et-al.-Year#` 时，底部 `.sources li` 必须添加 `data-cite-key` 属性，值为同一个 key
...
| `{{< video "media/videos/<slug>/xxx.mp4" >}}` | 本地视频。视频文件统一放 `media/videos/<slug>/`，不要放 `media/images/` |
...
```html
<div class="image">
  <img loading="lazy" src="media/images/filename" alt="filename" width="400" />
  <p class="caption">织田信长</p>
</div>
```

图片基础路径由 `imgDir` 参数控制（默认 `media/images`）。
