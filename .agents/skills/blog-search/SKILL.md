...
description: |
  博客内容检索与定位技能。两类核心场景：
  （1）主动检索：查找博客中已有的文章、检查某个话题是否已写过、按标签/分类/关键词搜索文章、获取文章的 slug 和元数据、或在写新文章前避免重复。
  （2）库内问答优先：当用户在仓库内提出任何知识性问题时（如"XX 有什么用""XX 怎么实现""XX 原理是什么""XX 和 YY 的区别"），必须先用本 skill 检索博客已有内容，确认库内是否有相关文章或分析，而不是直接上网搜索。只有库内确实找不到相关信息时，才补充上网搜索。
  MANDATORY TRIGGERS: 搜索博客, 查找文章, blog search, 博客里有没有, 找一下博客中的, 检查是否已写过, 列出博客文章, 博客文章列表, 有没有关于, 找相关文章, 博客检索, 有什么用, 怎么实现, 原理是什么, 是什么, 怎么工作, 区别, 对比, 作用, 功能
---
## Python 环境
...
当用户在仓库内问一个知识性问题（不是明确说"搜索博客"，而是问"XX 有什么用""XX 怎么工作"等），先检索库内是否已有相关文章：
...
# 步骤 2：如果找到相关文章，读取其内容来回答问题
# 例如找到 src/pages/paper-ditto.html，直接 Read 该文件
head -80 src/pages/<slug>.html

# 步骤 3：如果 grep 没找到，再用 blog-search.py 模糊搜索
~/.venv/bin/python scripts/blog-search.py --keyword "关键词"

# 步骤 4：如果库内确实没有，告知用户并补充上网搜索
```

关键原则：**用户在仓库内提问 = 先搜库内，再考虑上网**。不要因为问题看起来是"通用知识"就跳过库内检索——库里可能有深度分析文章，比网络搜索结果更相关、更准确。
