...
6. 在进入 html-blog 生成最终页面前，必须先补齐课程笔记 frontmatter：`description` 必须写成一句完整摘要；`sub_id` 默认自动分配，做法是先检索同一课程（同 `subcategory`）已有页面，取已使用 `sub_id` 的最大值，再给当前新笔记写入下一个整数。若用户明确指定章节号，或当前文章是在重写既有章节，才允许覆盖该自动编号。
