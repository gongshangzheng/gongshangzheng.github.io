...
name: site-dev
description: |
  静态博客站点开发指南。覆盖构建管线（build.js）、模板系统、shortcode 机制、
  processBody 替换规则、frontmatter 格式、布局规则、搜索系统、taxonomy 系统、
  测试规范、主题系统和 GitHub Actions 部署。
  用于修改构建逻辑、调试模板、新增 shortcode、修改 lib/ 模块或排查构建错误。
  触发词：build.js、构建、shortcode、模板、lib/、site build、构建错误、测试。
---
## Python 环境
...
5. **一次性迁移/清理不要写进长期构建脚本**：如迁移 `assets/images`、清理旧产物目录、批量修正历史文件等，只能用临时脚本或一次性命令完成；完成后删除临时脚本。`build.js` 只保留每次构建都必须执行的长期通用逻辑。
6. **交付前检查无关 diff**：执行 `git status --short` 和 `git diff --stat`。如果出现 `.cache`、`public/`、大量图片删除、无关历史文章批量变化等与当前任务无关的改动，先还原或向用户说明，禁止混入本次提交。
...
2. `rm public/` — 清理旧构建
3. `convertMediaImagesToWebP()` — 自动转换 `media/images/` 中的 PNG/JPG/PDF 为 WebP，删除原件，并更新文章中的图片引用
4. `copyDir(assets)` — 复制 CSS/JS/图片到 public/assets/
5. `copyDir(media)` — 复制 WebP/PDF/PPT/音频/视频到 public/media/
6. `copyDir(media → audio/)` — 兼容旧路径 `/audio/*.mp3`
...
关键字段：title, date, created_at, updated_at, tags, categories, subcategory, description, pin, mathjax, page_style, hero_title, hero_sub, hero_tagline, audio_src, draft, toc, notify。`pin: true` 会让文章进入首页 Pinned 置顶区。
