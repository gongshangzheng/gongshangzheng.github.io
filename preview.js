...
// ---------- HTTP server ----------
function serve(req, res, filePath, status = 200) {
...
const server = http.createServer((req, res) => {
  ...
  fs.stat(fp, (err, stats) => {
    if (err) {
      ...
      fs.stat(html, (e2, s2) => {
        if (!e2 && s2.isFile()) return serve(req, res, html);
        const idx = path.join(PUBLIC, p, 'index.html');
        fs.stat(idx, (e3, s3) => {
          if (!e3 && s3.isFile()) return serve(req, res, idx);
          res.writeHead(404, {'Content-Type':'text/plain'}); res.end('Not Found: ' + p);
        });
      });
      return;
    }
    if (stats.isFile()) return serve(req, res, fp);
    // Directory: try index.html inside it
    const idx = path.join(fp, 'index.html');
    fs.stat(idx, (e2, s2) => {
      if (!e2 && s2.isFile()) return serve(req, res, idx);
      // No index.html: try同名 .html (e.g. /categories/AI → categories/AI.html)
      const html = path.join(PUBLIC, p.replace(/\/$/, '') + '.html');
      fs.stat(html, (e3, s3) => {
        if (!e3 && s3.isFile()) return serve(req, res, html);
        res.writeHead(404, {'Content-Type':'text/plain'}); res.end('Not Found: ' + p);
      });
    });
  });
