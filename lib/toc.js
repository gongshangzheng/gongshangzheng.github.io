...
function buildTocSidebar(headings, posts = [], currentPost = null, taxonomy = defaultTaxonomy) {
  ...
  // Show the full site category tree (all top-level categories).
  // Non-current branches collapse by default via buildCategoryTree's
  // `collapsed: !containsCurrent`, so the sidebar stays navigable.
