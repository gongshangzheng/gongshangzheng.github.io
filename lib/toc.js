...
function renderTocNode(node) {
  ...
  html += `<a href="${node.href || `#${node.id}`}" data-level="${node.level}">${node.text}</a>`;
...
function buildCategoryTree(posts) {
  ...
  return Object.keys(catMap).sort().map(cat => ({
    ...
    collapsed: true,
    children: Object.keys(catMap[cat]).sort().map(subcat => ({
      level: 2,
...
function buildCategoryHtml(posts) {
...
function buildTocSidebar(headings, posts = []) {
  const tocContent = buildTocHtml(headings);
  const categoryContent = buildCategoryHtml(posts);
