...
function processHeadings(bodyHtml) {
  ...
  // --- Single combined regex: .ch-title divs, .section-title divs, AND h2-h6, processed in document order ---
  //   Group 1: div opening tag (ch-title)    Group 2: div content
  //   Group 3: div opening tag (section-title) Group 4: div content
  //   Group 5: h tag name                    Group 6: h attributes   Group 7: h content
  const processed = placeholderHtml.replace(
    /<(div[^>]*class="[^"]*\bch-title\b[^"]*"[^>]*)>(.*?)<\/div>|<(div[^>]*class="[^"]*\bsection-title\b[^"]*"[^>]*)>(.*?)<\/div>|<(h[2-6])([^>]*)>(.*?)<\/\5>/gi,
    (match, chOpen, chContent, secOpen, secContent, hTag, hAttrs, hContent) => {
      if (chOpen !== undefined) {
  ...
  return Object.keys(catMap).sort().map(cat => {
    const subcatNodes = Object.keys(catMap[cat]).sort().map(subcat => {
      const postsInSubcat = catMap[cat][subcat]
        .slice()
        .sort((a, b) => {
          // Sort by sub_id ascending (numeric), fall back to created_at descending
          if (typeof a.sub_id === 'number' && typeof b.sub_id === 'number') {
      ...
      return {
        ...
        level: 2,
        text: subcat,
        slug: taxonomy.getSubcategorySlug(cat, subcat),
    ...
    return {
      ...
      level: 1,
      text: cat,
      slug: taxonomy.getCategorySlug(cat),
      collapsed: !containsCurrentPost,
...
function findCurrentCategoryPath(tree) {
  for (const category of tree) {
    for (const subcategory of category.children || []) {
      if ((subcategory.children || []).some(post => post.active)) {
        return [category.text, subcategory.text];
      }
    }
  }
  return [];
...
function buildTocSidebar(headings, posts = [], currentPost = null, taxonomy = defaultTaxonomy) {
  ...
  const categoryContent = buildCategoryHtml(posts, currentPost, taxonomy);
