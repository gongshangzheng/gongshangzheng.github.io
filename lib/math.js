...
function protectHtmlBlocks(html, callback) {
  const blocks = [];
  const protectedHtml = String(html || '').replace(/<(pre|code|script|style)\b[\s\S]*?<\/\1>/gi, (block) => {
    const token = `@@HTML_BLOCK_${blocks.length}@@`;
    blocks.push(block);
    return token;
  });
  const processed = callback(protectedHtml);
  return processed.replace(/@@HTML_BLOCK_(\d+)@@/g, (_, i) => blocks[Number(i)] || '');
}

function protectEscapedDollars(html) {
  const dollars = [];
  const protectedHtml = String(html || '').replace(/\\\$/g, () => {
    const token = `@@ESCAPED_DOLLAR_${dollars.length}@@`;
    dollars.push('$');
    return token;
  });
  return {
    html: protectedHtml,
    restore: (value) => String(value || '').replace(/@@ESCAPED_DOLLAR_(\d+)@@/g, (_, i) => dollars[Number(i)] || '$'),
  };
}
...
function transformLatex(bodyHtml) {
  return protectHtmlBlocks(bodyHtml, (unblockedHtml) => {
    const escapedDollars = protectEscapedDollars(unblockedHtml);
    const displayMath = [];
    ...
    html = protectHtmlTags(html, (text) => {
      // #key# citations are processed before inline math.
      text = text.replace(/#([^#\n]+)#/g, (_, key) => `<cite>#${key.trim()}</cite>`);
    ...
    return escapedDollars.restore(html);
  });
}
