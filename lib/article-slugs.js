...
function ensureArticleSlugs(records) {
  ...
  const activeSourcePaths = new Set(
    (records || [])
      .map(record => record && record.sourcePath)
      .filter(sourcePath => typeof sourcePath === 'string' && sourcePath.trim())
  );

  for (const sourcePath of Object.keys(registry.articles)) {
    if (!activeSourcePaths.has(sourcePath)) {
      delete registry.articles[sourcePath];
      mutated = true;
    }
  }
  ...
  for (const record of records || []) {
    ...
    // slug 统一从源文件名生成：纯 ASCII 直接用，含中文的转拼音
    const baseName = path.basename(sourcePath, path.extname(sourcePath));
    const isPureAscii = /^[\x20-\x7E]+$/.test(baseName);
    const slug = nextAvailableSlug(isPureAscii ? baseName : toAsciiBase(baseName), usedSlugs);
    usedSlugs.add(slug);
