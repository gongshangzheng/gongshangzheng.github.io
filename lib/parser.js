...
function parseFrontmatter(content) {
  // Normalize CRLF / CR → LF so the delimiters below (which assume \n)
  // match regardless of git's autocrlf or editor line-ending settings.
...
function parseYamlBlock(frontmatter, body) {
  ...
  while (i < lines.length) {
    ...
    // Parse integers for certain keys
    if (/^\d+$/.test(val) && (key === 'sub_id' || key === 'sub_index' || key === 'order')) {
