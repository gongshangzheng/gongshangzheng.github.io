 ...
 * Stops at subcategory level (depth 2) - does not render deeper.
 */
function renderNestedChips(node, taxonomy, currentPath = []) {
  ...
  for (const name of childNames) {
    ...
    // Only render grandchildren if depth < 2 (stop at subcategory level)
    const grandChips = child.depth < 2 ? renderNestedChips(child, taxonomy, [...currentPath, name]) : '';
