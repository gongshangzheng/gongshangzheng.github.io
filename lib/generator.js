...
function buildArticles(paths, allPosts, buildContext, recentCount) {
  for (const file of fs.readdirSync(paths.pages)) {
    ...
    if (isIndex) {
      ...
      // Extract stats from bodyHtml and place outside wrap, after hero
      const { html: statsHtml, body: bodyAfterStats } = extractFirstDiv(bodyHtml, 'stats');
      bodyHtml = bodyAfterStats;
      // Check if bodyHtml already starts with a wrap div (from source)
      const startsWithWrap = /^\s*<div class="wrap">/i.test(bodyHtml);
      if (startsWithWrap) {
        // bodyHtml already has its own wrap — use it directly
