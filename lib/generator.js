...
function buildArticles(paths, allPosts, buildContext, recentCount) {
  for (const file of fs.readdirSync(paths.pages)) {
    ...
    if (isIndex) {
      ...
      // Check if bodyHtml already has a .wrap div BEFORE extracting stats.
      // We only strip the first .stats div if there's no .wrap to take care of it.
      const hasSourceWrap = /^\s*<div class="wrap">/i.test(bodyHtml);
      let statsHtml = '';
      if (!hasSourceWrap) {
        const { html, body } = extractFirstDiv(bodyHtml, 'stats');
        statsHtml = html;
        bodyHtml = body;
      }
      if (hasSourceWrap) {
        // bodyHtml starts with .wrap from source — use it directly.
        // stats (if any) is already inside bodyHtml, not extracted.
        contentHtml = `${buildHero(fm)}${statsHtml}${buildArticleMeta(fm, bodyHtml)}${bodyHtml}${buildArticleFooter(fm)}`;
      } else {
        contentHtml = `${buildHero(fm)}${statsHtml}<div class="${containerClass}">${buildArticleMeta(fm, bodyHtml)}${bodyHtml}${buildArticleFooter(fm)}</div>`;
      }
    }
