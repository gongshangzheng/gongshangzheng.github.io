...
function renderPostList(posts) {
  return '<ul class="post-list">\n' +
    posts.map(p => `  <li><span class="date">${formatDateTime(p.created_at || p.date)}</span><a href="${p.url}">${p.title}</a></li>`).join('\n') +
    '\n</ul>';
}
...
function buildArticleMeta(fm, bodyHtml) {
  ...
  var hasCreated = fm.created_at || fm.date;
  var createdVal = formatDateTime(fm.created_at || fm.date);
  var updDateOnly = fm.updated_at ? fm.updated_at.substring(0, 10) : '';
  var hasUpdated = fm.updated_at && updDateOnly !== (fm.date || '').substring(0, 10);
...
#search-results .search-meta { color: var(--text-muted); font-size: 0.85rem; margin-bottom: 16px; }</style>
    ...
    (function() {
      ...
      function fmtDate(val){if(!val)return'';var d=val.substring(0,10).replace(/-/g,'/');if(val.length>10){var t=val.substring(11,19);if(t)d+=' '+t;}return d;}
      input.addEventListener('input', function() {
        ...
        matched.forEach(function(p){ html += '<li><span class="date">' + fmtDate(p.created_at || p.date) + '</span><a href="./' + p.url + '">' + p.title + '</a></li>'; });
