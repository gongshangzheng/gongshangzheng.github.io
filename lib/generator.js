...
// Proper div extraction: find first <div class="X"> (with optional attrs after) and match closing </div>
function extractFirstDiv(html, className) {
  // Match opening tag: <div class="CLASSNAME" (with optional attrs) >
...
function buildArticles(paths, allPosts, buildContext, recentCount) {
  for (const file of fs.readdirSync(paths.pages)) {
    ...
    // Process LaTeX math: display math uses <div class="math-block">\[...\]</div> in source;
    // inline math uses <span class="math-inline">$...$</span>.  This avoids conflict with
    // the $key$ citation syntax (<cite>$key$</cite>).
    bodyHtml = bodyHtml.replace(
      /\\\[(.*?)\\\]/gs,
      function(_, latex) { return '<div class="math-block"><code class="math display">\\[' + latex + '\\]</code></div>'; }
    );
    bodyHtml = bodyHtml.replace(
      /\$([^\$\n]+?)\$/g,
      function(_, latex) { return '<span class="math-inline"><code class="math">$' + latex + '$</code></span>'; }
    );
    ...
    if (isIndex) {
      ...
      if (hasSourceWrap) {
        // Source has its own .wrap — stats outside, article-meta + footer inside wrap.
        ...
        // Insert footer before the wrap's closing </div>
        if (footerHtml) {
        ...
        // No source .wrap — stats goes outside wrap (full-width, flush under hero),
        // body + meta + footer go inside wrap.
