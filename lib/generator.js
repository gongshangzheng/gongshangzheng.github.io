...
function buildArticles(paths, allPosts, buildContext, recentCount) {
  for (const file of fs.readdirSync(paths.pages)) {
    ...
    // Store LaTeX in <code class="math"> so MathJax typesets via processClass.
    //
    ...
    bodyHtml = bodyHtml.replace(
      ...
      function(_, latex) {
        ...
        return '<div class="math-block" data-math="' + clean.replace(/"/g, '&quot;') + '"></div>';
      }
    );
    bodyHtml = bodyHtml.replace(
      /\\\[(.*?)\\\]/gs,
    ...
    bodyHtml = bodyHtml.replace(
      /\$([^\$\n]+?)\$/g,
      function(_, latex) {
        return '<span class="math-inline" data-math="' + latex.replace(/"/g, '&quot;') + '"></span>';
      }
    );
...
<script id="MathJax-script" src="https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-mml-chtml.js?config=TeX-MML-AM_CHTML" async></script>
<script>
  ...
  var timer = setInterval(function() {
    ...
    if (mj && mj.startup && mj.startup.promise) {
      mj.startup.promise.then(function() {
        ...
        if (els.length > 0) {
          mj.typesetPromise(els).then(function() {
            console.log('MathJax typeset done for', els.length, 'elements');
          }).catch(function(e) {
            console.error('MathJax error:', e);
          });
        } else {
          console.log('No math elements found');
        }
        clearInterval(timer);
      });
      tries++;
