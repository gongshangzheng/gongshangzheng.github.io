...
// Proper div extraction: find first <div class="X"> and match closing </div>
function extractFirstDiv(html, className) {
  ...
  return {
    html: html.substring(start, i),
    body: html.substring(0, start) + html.substring(i)
  };
}
...
function buildArticleMeta(fm, bodyHtml) {
  ...
  if (cats.length) {
    ...
    html += cats.map(c => `<a class="meta-tag" href="./categories/${c}.html">${c}</a>`).join('');
    html += '</span>';
  }
...
function buildArticles(paths, allPosts, buildContext, recentCount) {
  for (const file of fs.readdirSync(paths.pages)) {
    ...
    // Only apply replacements/shortcodes to .md files.
    // .html source files already contain processed HTML — pass through unchanged.
    if (ext === '.md') {
    ...
    if (isIndex) {
      ...
      // Always extract .stats div first, before checking for source .wrap.
      // This ensures .stats ends up in the same structural position as article content.
      const { html: statsHtml, body: bodyAfterStats } = extractFirstDiv(bodyHtml, 'stats');

      // Check if bodyAfterStats starts with a .wrap from source.
      const hasSourceWrap = /^\s*<div class="wrap">/i.test(bodyAfterStats);

      if (hasSourceWrap) {
        // Source has its own .wrap — keep stats (already extracted) outside it,
        // article-meta at the start of the source wrap alongside .stats.
        contentHtml = `${buildHero(fm)}${statsHtml}${buildArticleMeta(fm, bodyAfterStats)}${bodyAfterStats}${buildArticleFooter(fm)}`;
      } else {
        // No source .wrap — wrap everything (stats + body + meta) together.
        // article-meta goes inside the wrap so it has matching padding as content.
        contentHtml = `${buildHero(fm)}<div class="${containerClass}">${statsHtml}${buildArticleMeta(fm, bodyAfterStats)}${bodyAfterStats}${buildArticleFooter(fm)}</div>`;
      }
    }
    ...
    // Inject MathJax if mathjax: true in frontmatter
    // Use </head>\n as anchor to avoid matching </head> inside injectStr itself
    if (fm.mathjax) {
      const headEndIdx = page.indexOf('</head>\n');
      if (headEndIdx >= 0) {
        page = page.slice(0, headEndIdx) + `<script src="https://polyfill.io/v3/polyfill.min.js?features=es6"></script>
...
window.MathJax = {
  ...
  options: { skipHtmlTags: ['script', 'noscript', 'style', 'textarea', 'pre', 'code'] }
    ...
    // Inject audio if audio_src is set
