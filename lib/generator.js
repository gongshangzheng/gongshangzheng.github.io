...
function buildTaxonomyPages(paths, allPosts, buildContext) {
  ...
  for (const tag of tagNames) {
    ...
    // collide with 'Mamba' tag, producing the wrong content (ic-entropy instead of correct)
    const tagSlug = taxonomySlug(tag);
    if (tagSlug === 'mamba') {
      // This slug would collide with Mamba.html — skip it
      console.log('SKIP mamba tag (collides with Mamba) at tagNames index:', tagNames.indexOf(tag));
      continue;
    }
    ...
    if (tag === 'Mamba') {
      console.log('DEBUG MAMBA content generated len:', content.length, 'has_marker:', content.includes('MAMBA_LEN_8056'));
      console.log('DEBUG MAMBA content has a-survey:', content.includes('a-survey'));
    }
    ...
    if (tag === 'Mamba') {
      const outPath = path.join(paths.public, `tags/${taxonomySlug(tag)}.html`);
      console.log('DEBUG outPath:', outPath);
      console.log('DEBUG outPath realpath:', fs.realpathSync(outPath));
      const afterWrite = fs.readFileSync(outPath, 'utf8');
      console.log('AFTER WRITE len:', afterWrite.length, 'has-marker:', afterWrite.includes('MAMBA_LEN_8056'), 'a-survey:', afterWrite.includes('a-survey'));
      const actualOnDisk = fs.readFileSync('/Users/zhengxinyu/gongshangzheng.github.io/public/tags/Mamba.html', 'utf8');
      console.log('ACTUAL DISK len:', actualOnDisk.length, 'has-marker:', actualOnDisk.includes('MAMBA_LEN_8056'), 'a-survey:', actualOnDisk.includes('a-survey'));
    }
  }
  const catMap = {};
  for (const p of allPosts) {
