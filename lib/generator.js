...
// Estimate reading time from HTML content (mixed CJK + English)
function estimateReadingTime(html) {
  const text = html.replace(/<[^>]*>/g, '').replace(/\s+/g, '');
  // Mixed content: ~500 chars per minute for CJK, ~200 words for English
  // Simplified: total non-space chars / 400, min 1 min
...
function buildArticles(paths, allPosts, buildContext, recentCount) {
  for (const file of fs.readdirSync(paths.pages)) {
    ...
    if (isIndex) {
      ...
      // Extract stats from bodyHtml to place outside wrap
      const statsMatch = bodyHtml.match(/<div class="stats">[\s\S]*?<\/div>/);
      const statsHtml = statsMatch ? statsMatch[0] : '';
      if (statsMatch) {
        bodyHtml = bodyHtml.replace(statsHtml, '');
      }
