...
function buildArticles(paths, allPosts, buildContext, recentCount) {
  for (const file of fs.readdirSync(paths.pages)) {
    ...
    if (isIndex) {
      ...
      if (hasSourceWrap) {
        // Source has its own .wrap — stats outside, article-meta inside wrap.
        const metaHtml = buildArticleMeta(fm, bodyAfterStats);
        const bodyWithMeta = bodyAfterStats.replace(
          /<div class="wrap">/i,
          `<div class="wrap">${metaHtml}`
        );
        contentHtml = `${buildHero(fm)}${statsHtml}${bodyWithMeta}${buildArticleFooter(fm)}`;
      } else {
