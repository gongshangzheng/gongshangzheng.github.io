...
const { filterGraphByScope } = require('./post-graph-db');
...
function processBody(bodyHtml, opts) {
  ...
  // Normalize code blocks that were migrated as <p><pre>...</pre></p>.
  str = str.replace(/<p>\s*(<pre[\s\S]*?<\/pre>)\s*<\/p>/gi, '$1');

  // Save code blocks as placeholders
  const codeBlocks = [];
  str = str.replace(/<pre[\s\S]*?<\/pre>/gi, (match) => {
    const placeholder = '__CODEBLOCK_' + codeBlocks.length + '__';
    codeBlocks.push(match.replace(/&lt;\/p&gt;\s*&lt;p&gt;/g, '\n'));
    return placeholder;
  });

  // Save script blocks as placeholders to avoid corrupting JS operators
  const scriptBlocks = [];
  str = str.replace(/<script[\s\S]*?<\/script>/gi, (match) => {
    const placeholder = '__SCRIPTBLOCK_' + scriptBlocks.length + '__';
    scriptBlocks.push(match);
    return placeholder;
  });

  // Save MathJax blocks/spans as placeholders to avoid corrupting formula operators.
  // Display math must be completely opaque to later Markdown/wiki/link replacements.
  const mathBlocks = [];
  str = str.replace(/<div\s+class="math-block"(?:\s+style="[^"]*")?>[\s\S]*?<\/div>/gi, (match) => {
  ...
  const mathSpans = [];
  str = str.replace(/<span\s+class="math-(?:inline|display)"[\s\S]*?<\/span>/gi, (match) => {
    const placeholder = '__MATHSPAN_' + mathSpans.length + '__';
  ...
  function applyInlineMarkdown(content) {
    const inlineCodeBlocks = [];
    let result = content.replace(/`([^`]+)`/g, function(match, code) {
    ...
    result = result
      .replace(/\[([^\]]+)\]\(([^)]+)\)/g, function(match, text, url) {
        return '<a href="' + url + '">' + applyInlineMarkdown(text) + '</a>';
      })
      .replace(/\*\*(?![<\s])([^*]+?)(?![<\s])\*\*/g, '<strong>$1</strong>')
      .replace(/(?<!\*)\*([^*]+?)\*(?!\*)/g, '<strong>$1</strong>')
      .replace(/(?<![=])==([^=]+)==(?![=])/g, '<mark>$1</mark>');

    inlineCodeBlocks.forEach(function(block, index) {
      result = result.replace('__INLINE_CODE_' + index + '__', block);
    });
    return result;
  }

  function applyInlineMarkdownToTextNodes(html) {
    // Highlight (==...==) must run on the full string before splitting by tags,
    // because the content between == may contain inline HTML (e.g. <cite> from
    // citation conversion). Splitting first would break the == pairs across
    // separate text nodes and prevent matching.
    html = html.replace(/(?<![=])==((?:(?!==)[\s\S])+)==(?![=])/g, '<mark>$1</mark>');

    return html.split(/(<[^>]+>)/g).map(function(part) {
      if (!part || part.charAt(0) === '<') return part;
      return applyInlineMarkdown(part);
    }).join('');
  }

  // Markdown unordered list paragraphs:
  // <p>- item\n- item</p> → <ul><li>item</li><li>item</li></ul>
  str = str.replace(/<p>([\s\S]*?)<\/p>/g, function(match, content) {
    const lines = content.split(/\n+/).map(line => line.trim()).filter(Boolean);
    if (!lines.length || !lines.every(line => /^-\s+/.test(line))) return match;

    const items = lines.map(function(line) {
      return '<li>' + applyInlineMarkdownToTextNodes(line.replace(/^-\s+/, '')) + '</li>';
    }).join('');
    return '<ul>' + items + '</ul>';
  });

  // Markdown blockquote paragraphs:
  // <p>> text</p> → <blockquote><p>text</p></blockquote>
  str = str.replace(/<p>([\s\S]*?)<\/p>/g, function(match, content) {
    const lines = content.split(/\n+/).map(line => line.trim()).filter(Boolean);
    if (!lines.length || !lines.every(line => /^>\s?/.test(line))) return match;

    const paragraphs = lines.map(function(line) {
      const quoteContent = line.replace(/^>\s?/, '').replace(/^####\s+/, '');
      return '<p>' + applyInlineMarkdownToTextNodes(quoteContent) + '</p>';
    }).join('');
    return '<blockquote>' + paragraphs + '</blockquote>';
  });

  // Inline Markdown for regular text nodes only. Never touch HTML tag attributes.
  str = applyInlineMarkdownToTextNodes(str);
  ...
  // Internal links: [[path#anchor | alias]] -- only match if not starting with @ or 'list ' (handled separately)
  const internalRe = /\[\[((?!@)(?!list\s)[^\]|]+)?(?:\s*#\s*([^\]|]+))?(?:\|\s*([^\]]+))?\]\]/g;
  let internalMatch;
  ...
  //
  // Critical: the capture groups must NOT cross block boundaries. Using a bare
  // [\s\S]*? lets a single <li>/<p> opener match a </li>/</p> many blocks later,
  // swallowing intervening content and emitting stray </dd></p></dl> tags.
  // INNER only matches characters that are not the start/end of a li/p/div block,
  // so each rule stays within one block.
  const INNER = '(?:(?!<\\/?(?:li|p|div)\\b)[\\s\\S])*?';
  str = str.replace(new RegExp('<li>(' + INNER + ')<strong>([^<]+)<\\/strong>(' + INNER + ')\\s::\\s(' + INNER + ')<\\/li>', 'gi'),
    function(_, pre, term, post, def) {
      return '<dl class="term-list"><li class="term"><dt>' + pre + '<strong>' + term + '</strong>' + post + '</dt><dd>' + def + '</dd></li></dl>';
    }
  );
  str = str.replace(new RegExp('<p>(' + INNER + ')<strong>([^<]+)<\\/strong>(' + INNER + ')\\s::\\s(' + INNER + ')<\\/p>', 'gi'),
    function(_, pre, term, post, def) {
      return '<dl class="term-list"><p class="term"><dt>' + pre + '<strong>' + term + '</strong>' + post + '</dt><dd>' + def + '</dd></p></dl>';
    }
  );
  str = str.replace(new RegExp('<li>(' + INNER + ')<em>([^<]+)<\\/em>(' + INNER + ')\\s::\\s(' + INNER + ')<\\/li>', 'gi'),
    function(_, pre, term, post, def) {
      return '<dl class="term-list"><li class="term"><dt>' + pre + '<em>' + term + '</em>' + post + '</dt><dd>' + def + '</dd></li></dl>';
    }
  );
  str = str.replace(new RegExp('<p>(' + INNER + ')<em>([^<]+)<\\/em>(' + INNER + ')\\s::\\s(' + INNER + ')<\\/p>', 'gi'),
    function(_, pre, term, post, def) {
      return '<dl class="term-list"><p class="term"><dt>' + pre + '<em>' + term + '</em>' + post + '</dt><dd>' + def + '</dd></p></dl>';
    }
  );
  str = str.replace(new RegExp('<li>(' + INNER + ')\\s::\\s(' + INNER + ')<\\/li>', 'gi'),
    function(_, term, def) {
      return '<dl class="term-list"><li class="term"><dt>' + term + '</dt><dd>' + def + '</dd></li></dl>';
    }
  );
  str = str.replace(new RegExp('<p>(' + INNER + ')\\s::\\s(' + INNER + ')<\\/p>', 'gi'),
    function(_, term, def) {
  ...
  scriptBlocks.forEach(function(block, i) {
    str = str.replace('__SCRIPTBLOCK_' + i + '__', block);
  });

  // Restore MathJax blocks and spans with function replacements.
  // Plain string replacements treat $$ as an escape sequence and collapse it to $.
  mathBlocks.forEach(function(block, i) {
    str = str.replace('__MATHBLOCK_' + i + '__', function() { return block; });
  });
  mathSpans.forEach(function(block, i) {
    str = str.replace('__MATHSPAN_' + i + '__', function() { return block; });
  });

  return str;
}

function processShortcodes(bodyHtml, allPosts, renderSortableList, postGraph, renderPostGraph) {
  let str = bodyHtml;

  // sortable-list shortcode:
  //   {{< sortable-list >}}                           — all posts
  //   {{< sortable-list "tags/AI" >}}                — filter by tag
  //   {{< sortable-list "categories/数学/线性代数" >}} — filter by category/subcategory
  //   {{< sortable-list "categories/AI/动作识别/宠物动作识别" >}} — filter by subsubcategory
  if (allPosts && Array.isArray(allPosts) && typeof renderSortableList === 'function') {
    str = str.replace(/\{\{<\s*sortable-list\s*([^>]*)\s*>\}\}/g, function(match, attrs) {
      var a = parseShortcodeAttrs(attrs);
      // Positional first arg or path= param
      var raw = (a.path || '').replace(/^['"]|['"]$/g, '');
      if (!raw) {
        // Try positional: {{< sortable-list "tags/AI" >}}
        var posMatch = attrs.match(/^\s*['"]([^'"]+)['"]|^\s*(\S+)/);
        if (posMatch) raw = posMatch[1] || posMatch[2] || '';
      }
      var filtered = allPosts;
      var scopedToSubcategory = false;
      if (raw) {
        var parts = raw.split('/').filter(Boolean);
        var kind = parts[0]; // 'tags' or 'categories'
        if (kind === 'tags' && parts[1]) {
          var tagVal = parts[1];
          filtered = filtered.filter(function(p) { return (p.tags || []).indexOf(tagVal) !== -1; });
        } else if (kind === 'categories' && parts[1]) {
          // Path prefix match using categoryPath
          var scopePath = parts.slice(1);
          filtered = filtered.filter(function(p) {
            var cp = p.categoryPath || [];
            if (cp.length < scopePath.length) return false;
            for (var i = 0; i < scopePath.length; i++) {
              if (cp[i] !== scopePath[i]) return false;
            }
            return true;
          });
          // Scoped to subcategory if path has 2+ segments
          scopedToSubcategory = scopePath.length >= 2;
        }
      }
      // Also support explicit named params as fallback
      if (a.tag) filtered = filtered.filter(function(p) { return (p.tags || []).indexOf(a.tag) !== -1; });
      if (a.category) {
        var catPath = [a.category];
        if (a.subcategory) catPath.push(a.subcategory);
        if (a.subsubcategory) catPath.push(a.subsubcategory);
        filtered = filtered.filter(function(p) {
          var cp = p.categoryPath || [];
          if (cp.length < catPath.length) return false;
          for (var i = 0; i < catPath.length; i++) {
            if (cp[i] !== catPath[i]) return false;
          }
          return true;
        });
        if (a.subcategory) scopedToSubcategory = true;
      }
      // Default sort: explicit sort="sub_id" wins; otherwise series order only when
      // the list is scoped to a single subcategory (or deeper).
      var explicitSort = (a.sort || '').replace(/^['"]|['"]$/g, '');
      var defaultSort = explicitSort || (scopedToSubcategory ? 'sub_id' : '');
      return renderSortableList(filtered, { linkPrefix: '../', defaultSort: defaultSort });
    });
  }

  // post-graph shortcode:
  //   {{< post-graph "tags/AI" >}}
  //   {{< post-graph "categories/课程/数字信号处理" height=620 depth=1 >}}
  if (postGraph && typeof renderPostGraph === 'function') {
    str = str.replace(/\{\{<\s*post-graph\s*([^>]*)\s*>\}\}/g, function(match, attrs) {
      var a = parseShortcodeAttrs(attrs);
      var raw = (a.path || '').replace(/^['"]|['"]$/g, '');
      if (!raw) {
        var posMatch = attrs.match(/^\s*['"]([^'"]+)['"]|^\s*(\S+)/);
      ...
      if (!raw && a.tag) raw = 'tags/' + a.tag;
      if (!raw && a.category) {
        raw = 'categories/' + a.category;
        if (a.subcategory) raw += '/' + a.subcategory;
        if (a.subsubcategory) raw += '/' + a.subsubcategory;
      }

      var filteredGraph = filterGraphByScope(postGraph, raw, { depth: a.depth || 0 });
      return renderPostGraph(filteredGraph, {
        height: a.height,
        labels: a.labels,
        hide_isolated: a.hide_isolated,
        title: a.title || '文章关系图',
      });
    });
  }
  ...
  str = str.replace(/\{\{\<\s*details\s+([^>]+)\s*>\}\}([\s\S]*?)\{\{\<\s*\/details\s*>\}\}/g, function(match, attrs, content) {
    ...
    var summaryMatch = attrs.match(/summary\s*=\s*"([^"]+)"/i) || attrs.match(/summary\s*=\s*(\S+)/i);
  ...
  // div.mermaid: support inline <div class="mermaid"...>mermaid code</div> syntax
  str = str.replace(/<div\s+class="mermaid"[^>]*>([\s\S]*?)<\/div>/g, function(match, code) {
    // Trim leading/trailing whitespace and normalize indentation
    var trimmed = code.replace(/^\n*/, '').replace(/\n*$/, '');
    var lines = trimmed.split('\n');
    var minIndent = Infinity;
