...
function processBody(bodyHtml, opts) {
  ...
  str = str.replace(/<li>([^<]*?)<strong>([^<]+)<\/strong>([^<]*?)::([^<]*)<\/li>/gi,
    function(_, pre, term, post, def) {
      return '<li class="term"><dt>' + pre + '<strong>' + term + '</strong>' + post + '</dt><dd>' + def + '</dd></li>';
    }
  );
  str = str.replace(/<p>([^<]*?)<strong>([^<]+)<\/strong>([^<]*?)::([^<]*)<\/p>/gi,
    function(_, pre, term, post, def) {
      return '<p class="term"><dt>' + pre + '<strong>' + term + '</strong>' + post + '</dt><dd>' + def + '</dd></p>';
    }
  );
  str = str.replace(/<li>([^<]*?)<em>([^<]+)<\/em>([^<]*?)::([^<]*)<\/li>/gi,
    function(_, pre, term, post, def) {
      return '<li class="term"><dt>' + pre + '<em>' + term + '</em>' + post + '</dt><dd>' + def + '</dd></li>';
    }
  );
  str = str.replace(/<p>([^<]*?)<em>([^<]+)<\/em>([^<]*?)::([^<]*)<\/p>/gi,
    function(_, pre, term, post, def) {
      return '<p class="term"><dt>' + pre + '<em>' + term + '</em>' + post + '</dt><dd>' + def + '</dd></p>';
    }
  );
  str = str.replace(/<li>([^<]*)::([^<]*)<\/li>/gi,
    function(_, term, def) {
      var term_clean = term.replace(/<\/?strong>/g, '').replace(/<\/?em>/g, '');
      return '<li class="term"><dt>' + term + '</dt><dd>' + def + '</dd></li>';
    }
  );
  str = str.replace(/<p>([^<]*)::([^<]*)<\/p>/gi,
    function(_, term, def) {
      var term_clean = term.replace(/<\/?strong>/g, '').replace(/<\/?em>/g, '');
      return '<p class="term"><dt>' + term + '</dt><dd>' + def + '</dd></p>';
    }
  );
