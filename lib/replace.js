...
function processBody(bodyHtml, opts) {
  ...
  // Use negative lookbehind/lookahead to avoid matching === in JavaScript
  ...
  // Taxonomy list shortcode: [[list category=AI]] or [[list tag=Mamba]]
  // Requires allPosts in opts to resolve taxonomy articles.
  if (opts && opts.allPosts) {
    var allPosts = opts.allPosts;
    var taxonomyListRe = /\[\[\s*list\s+(category|tag|subcategory)\s*=\s*([^\]]+?)\s*\]\]/gi;
    str = str.replace(taxonomyListRe, function(match, type, name) {
      name = name.trim();
      var filtered;
      if (type.toLowerCase() === 'category') {
        filtered = allPosts.filter(function(p) { return p.categories && p.categories.indexOf(name) >= 0; });
      } else if (type.toLowerCase() === 'tag') {
        filtered = allPosts.filter(function(p) { return p.tags && p.tags.indexOf(name) >= 0; });
      } else if (type.toLowerCase() === 'subcategory') {
        filtered = allPosts.filter(function(p) { return p.subcategory === name; });
      } else {
        return match;
      }
      if (filtered.length === 0) return '<p class="taxonomy-list-empty">暂无文章。</p>';
      // Sort by created_at descending
      filtered.sort(function(a, b) { return (b.created_at || '').localeCompare(a.created_at || ''); });
      var label = type === 'category' ? '分类' : type === 'tag' ? '标签' : '子分类';
      var html = '<div class="taxonomy-list-block">';
      html += '<div class="taxonomy-list-header">' + label + '：' + escapeHtml(name) + '（' + filtered.length + ' 篇）</div>';
      html += '<ul class="post-list">';
      filtered.forEach(function(p) {
        var date = p.created_at ? p.created_at.substring(0, 10).replace(/-/g, '/') : '';
        html += '<li><span class="date">' + date + '</span><a href="' + p.url + '">' + escapeHtml(p.title) + '</a></li>';
      });
      html += '</ul></div>';
      return html;
    });
...
function processShortcodes(bodyHtml, allPosts, renderSortableList) {
  let str = bodyHtml;
  ...
  if (allPosts && Array.isArray(allPosts) && typeof renderSortableList === 'function') {
    str = str.replace(/\{\{<\s*sortable-list\s*([^>]*)\s*>\}\}/g, function(match, attrs) {
      ...
      if (a.subcategory) filtered = filtered.filter(function(p) { return (p.subcategory || '') === a.subcategory; });
      return renderSortableList(filtered, { linkPrefix: '../' });
    });
  }
