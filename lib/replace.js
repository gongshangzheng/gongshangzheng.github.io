...
function processBody(bodyHtml, opts) {
  ...
  // Wiki image: ![[img | width # caption]]
  str = str.replace(/!\[\[\s*([^\]]+)\s*\|?\s*(\d*)\s*#?\s*([^\]\s]*?)\s*\]\]/g, (match, src, width, caption) => {
  ...
  for (const match of wikiLinks) {
    ...
    const text = match[2];
    const display = match[3] || text;
    const url = 'https://' + country + '.wikipedia.org/wiki/' + encodeURIComponent(text);
    // Use greedy match: avoids escaping issues with country codes like .en
    const linkRegex = /\[\[\s*wiki\.?\S*\s+[^\]]+\s*(?:\|[^\]]+)?\s*\]\]/gi;
    str = str.replace(linkRegex, '<a href="' + url + '" class="search-wiki" target="_blank" rel="noopener">' + display + '</a>');
  }
  ...
  while ((arxivMatch = arxivRe.exec(str)) !== null) {
    ...
    const suffix = arxivMatch[3] || '';
    const display = arxivMatch[4] || id;
    const url = 'https://arxiv.org/' + subpage + '/' + id + suffix;
  ...
  while ((githubMatch = githubRe.exec(str)) !== null) {
    const text = githubMatch[1];
    const display = githubMatch[2] || text;
