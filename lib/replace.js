...
function processBody(bodyHtml, opts) {
  ...
  for (const match of wikiLinks) {
    ...
    const regex = new RegExp('\\[\\[\\s*wiki\\.?' + escapedCountry + '\\s+' + escapedText + '(?:\\s*\\|\\s*' + escapedDisplay + ')?\\]\\]', 'gi');
    str = str.replace(regex, '<a href="' + url + '" class="search-wiki" target="_blank" rel="noopener">' + display + '</a>');
  }
