...
function processShortcodes(bodyHtml) {
  ...
  // local PDF page reference shortcode
  // {{< pdf "course/lecture.pdf" page=12 title="说明" >}}
  str = str.replace(/\{\{\<\s*pdf\s+"([^"]+)"([^>]*)\s*>\}\}/g, function(match, src, attrs) {
    var pageMatch = attrs.match(/\bpage\s*=\s*(\d+)/i);
    var titleMatch = attrs.match(/\btitle\s*=\s*"([^"]+)"/i);
    var page = pageMatch ? pageMatch[1] : '1';
    var title = titleMatch ? titleMatch[1] : src;
    var url = src.match(/^(https?:)?\//) ? src : './media/' + src.replace(/^\.?\/?media\//, '');
    var pageUrl = url + '#page=' + page;
    return '<div class="doc-ref pdf-ref"><div class="doc-ref-head"><span class="doc-ref-type">PDF</span><strong>' + title + '</strong><span class="doc-ref-page">p.' + page + '</span></div><iframe src="' + pageUrl + '" loading="lazy" width="100%" height="520" frameborder="0"></iframe><p class="caption"><a href="' + pageUrl + '" target="_blank" rel="noopener">打开 PDF 第 ' + page + ' 页</a></p></div>';
  });

  // local PPT/PPTX page reference shortcode
  // {{< ppt "course/lecture.pptx" page=8 title="说明" >}}
  // Note: browsers do not support reliable direct page/slide anchors for PPT/PPTX, so render a reference card.
  str = str.replace(/\{\{\<\s*ppt\s+"([^"]+\.(?:pptx?|PPTX?))"([^>]*)\s*>\}\}/g, function(match, src, attrs) {
    ...
    var title = titleMatch ? titleMatch[1] : src;
    var url = src.match(/^(https?:)?\//) ? src : './media/' + src.replace(/^\.?\/?media\//, '');
    return '<div class="doc-ref ppt-ref"><div class="doc-ref-head"><span class="doc-ref-type">PPT</span><strong>' + title + '</strong><span class="doc-ref-page">slide ' + page + '</span></div><p>引用课件：' + title + '，第 ' + page + ' 页。</p><p class="caption"><a href="' + url + '" target="_blank" rel="noopener">打开 / 下载 PPT 文件</a></p></div>';
  });
