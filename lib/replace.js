...
function processShortcodes(bodyHtml, allPosts, renderSortableList, postGraph, renderPostGraph) {
  ...
  if (allPosts && Array.isArray(allPosts) && typeof renderSortableList === 'function') {
    str = str.replace(/\{\{<\s*sortable-list\s*([^>]*)\s*>\}\}/g, function(match, attrs) {
      ...
      if (raw) {
        ...
        var sub = parts[2] || '';
        if (kind === 'tags' && val) {
          filtered = filtered.filter(function(p) { return (p.tags || []).indexOf(val) !== -1; });
        } else if (kind === 'categories' && val) {
          filtered = filtered.filter(function(p) { return (p.categories || []).indexOf(val) !== -1; });
          if (sub) {
            filtered = filtered.filter(function(p) { return (p.subcategory || '') === sub; });
            scopedToSubcategory = true;
          }
        }
      }
      // Also support explicit named params as fallback
      if (a.tag) filtered = filtered.filter(function(p) { return (p.tags || []).indexOf(a.tag) !== -1; });
      if (a.category) filtered = filtered.filter(function(p) { return (p.categories || []).indexOf(a.category) !== -1; });
      if (a.subcategory) { filtered = filtered.filter(function(p) { return (p.subcategory || '') === a.subcategory; }); scopedToSubcategory = true; }
      // Default sort: explicit sort="sub_id" wins; otherwise series order only when
      // the list is scoped to a single subcategory.
  ...
  if (postGraph && typeof renderPostGraph === 'function') {
    str = str.replace(/\{\{<\s*post-graph\s*([^>]*)\s*>\}\}/g, function(match, attrs) {
      ...
      if (!raw && a.category) raw = 'categories/' + a.category + (a.subcategory ? '/' + a.subcategory : '');
