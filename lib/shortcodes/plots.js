...
function processPlotShortcodes(html) {
  ...
  // 1. Open+close: body must not contain another {{< functionplot
  str = str.replace(/\{\{<\s*functionplot\s*([^>]*)>\}\}((?:(?!\{\{<\s*\/?functionplot)[\s\S])*?)\{\{<\s*\/functionplot\s*>\}\}/g, function(match, attrsStr, body) {
    return renderFunctionPlot(parseShortcodeAttrs(attrsStr), body, match);
  });

  // 2. Self-closing
  str = str.replace(/\{\{<\s*functionplot\s*([^>]*)>\}\}/g, function(match, attrsStr) {
    return renderFunctionPlot(parseShortcodeAttrs(attrsStr), '', match);
  });
