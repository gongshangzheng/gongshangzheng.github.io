...
function ensureCategorySlug(map, name) {
  ...
  // Check translation registry first (exact match)
  if (typeof categoryNames[name] === 'string' && categoryNames[name].trim()) {
    ...
    if (translated && !Array.from(Object.values(map)).includes(translated)) {
      map[name] = translated;
      return translated;
    }
  }
...
function ensureEntry(map, key) {
  // For tags — no translation registry, pure pinyin fallback
