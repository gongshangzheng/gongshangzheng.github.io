...
const tests = {
  ...
  'processHeadings: headings inside content boxes are excluded from TOC': () => {
    const html = `
      <div class="ch-title">Main Chapter</div>
      <div class="def-box"><h3>Definition Title</h3><p>Definition body.</p></div>
      <div class="theorem-box"><h3>Theorem Title</h3><p>Theorem body.</p></div>
      <div class="example-box"><h4>Example Detail</h4><p>Example body.</p></div>
      <h3>Visible Section</h3>
    `;
    const { html: out, headings } = processHeadings(html);
    assert.equal(headings.length, 2);
    assert.deepEqual(headings.map((heading) => heading.text), ['Main Chapter', 'Visible Section']);
    assert(!out.includes('id="Definition-Title"'));
    assert(!out.includes('id="Theorem-Title"'));
    assert(!out.includes('id="Example-Detail"'));
  },
