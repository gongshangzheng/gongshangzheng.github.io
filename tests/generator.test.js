...
const { REGISTRY_PATH, ensureArticleSlugs, getArticleSlug } = require('../lib/article-slugs');
const { transformLatex, transformMarkdownTables, transformTableCaptionScroll, buildArticles, taxonomySlug, categoryUrl, subcategoryUrl, renderSortableList, buildIndex } = require('../lib/generator');
...
const tests = {
  ...
  'transformMarkdownTables: converts preceding table caption paragraph to italic': () => {
    const input = '<p>表 1：IGA 与 GIFT 的核心差异。</p>\n<p>| 特性 | IGA | GIFT |</p>\n<p>|------|-----|------|</p>\n<p>| 解场样条空间 | 与几何相同 | 几何无关 |</p>';
    const html = transformMarkdownTables(input);

    assert(html.includes('<div class="table-wrap">'));
    assert(!html.includes('<caption>'));
    assert(html.includes('<p><em>表 1：IGA 与 GIFT 的核心差异。</em></p>'));
    assert(html.includes('<thead><tr><th>特性</th><th>IGA</th><th>GIFT</th></tr></thead>'));
    assert(html.includes('<td>解场样条空间</td>'));
  },

  'transformTableCaptionScroll: converts caption to italic paragraph': () => {
    const input = '<div class="table-wrap wide">\n<table>\n<caption>表 1：宽表说明。</caption>\n<thead><tr><th>A</th><th>B</th></tr></thead>\n</table>\n</div>';
    const html = transformTableCaptionScroll(input);

    assert(html.includes('<div class="table-wrap wide">'));
    assert(html.includes('<div class="table-scroll">'));
...
<body><p>Content</p></body></html>`;
  ...
  'article slug registry: chinese titles convert to pinyin and persist by source path': () => {
    ...
    const originalRegistry = fs.existsSync(REGISTRY_PATH) ? fs.readFileSync(REGISTRY_PATH, 'utf8') : null;
  ...
  'article slug registry: removes entries for deleted source paths': () => {
    const cwd = process.cwd();
    const originalRegistry = fs.existsSync(REGISTRY_PATH) ? fs.readFileSync(REGISTRY_PATH, 'utf8') : null;
    process.chdir(path.resolve(__dirname, '..'));
    try {
      fs.writeFileSync(REGISTRY_PATH, JSON.stringify({
        version: 1,
        articles: {
          'src/pages/active.html': { title: 'Active', slug: 'active' },
          'src/pages/deleted.html': { title: 'Deleted', slug: 'deleted' }
        }
      }, null, 2));
      const registry = ensureArticleSlugs([
        { sourcePath: 'src/pages/active.html', title: 'Active' }
      ]);
      assert.equal(getArticleSlug(registry, 'src/pages/active.html'), 'active');
      assert.equal(getArticleSlug(registry, 'src/pages/deleted.html'), '');
    } finally {
      if (originalRegistry === null) {
        fs.rmSync(REGISTRY_PATH, { force: true });
      } else {
  ...
  'buildIndex: includes pin flag for homepage pinned section': () => {
    const tempDir = '/tmp/post-index-pin-test-' + Date.now();
    fs.mkdirSync(tempDir, { recursive: true });
    try {
      buildIndex({ public: tempDir }, [
        { title: 'Pinned Post', slug: 'pinned-post', pin: true, created_at: '2026-01-02', updated_at: '2026-01-03' },
        { title: 'Normal Post', slug: 'normal-post', pin: false, created_at: '2026-01-01', updated_at: '2026-01-01' }
      ]);
      const index = JSON.parse(fs.readFileSync(path.join(tempDir, 'post-index.json'), 'utf8'));
      const pinnedPost = index.find(function(post) { return post.slug === 'pinned-post'; });
      const normalPost = index.find(function(post) { return post.slug === 'normal-post'; });
      assert.equal(pinnedPost.pin, true);
      assert.equal(normalPost.pin, false);
    } finally {
...
// ===== MathJax rendering =====
  ...
  'MathJax: escaped dollar is rendered as literal dollar': () => {
    const result = transformLatex('<p>Price is \\$5 and math $x_1$ still works.</p>');
    assert(result.includes('<p>Price is $5 and math <span class="math-inline">$x_1$</span> still works.</p>'),
      'escaped dollar should become literal dollar without triggering inline math');
  },

  'MathJax: pre code blocks are opaque to dollar math': () => {
    const input = '<pre><code class="language-bash">export PKG_CONFIG_PATH="$(brew --prefix opus)/lib/pkgconfig:$PKG_CONFIG_PATH"</code></pre><p>Math $x_1$ works.</p>';
    const result = transformLatex(input);
    assert(result.includes('<pre><code class="language-bash">export PKG_CONFIG_PATH="$(brew --prefix opus)/lib/pkgconfig:$PKG_CONFIG_PATH"</code></pre>'),
      'shell command inside pre/code must remain unchanged');
    assert(result.includes('<span class="math-inline">$x_1$</span>'),
      'math outside pre/code should still be transformed');
  },
  ...
  'Citation: hash syntax becomes cite element before inline math': () => {
    const result = transformLatex('<p>Reference #CatV2TON# and #Han et al., 2024# with formula $x_1$.</p>');
    assert(result.includes('<cite>#CatV2TON</cite>'),
      '#CatV2TON# should become cite');
    assert(result.includes('<cite>#Han et al., 2024</cite>'),
      '#Han et al., 2024# should become cite');
    assert(result.includes('<span class="math-inline">$x_1$</span>'),
      '$x_1$ should remain math');
  },
  ...
  'Term syntax: works when definition contains inline MathJax span': () => {
    ...
    const result = processBody(input, { imgDir: 'media/images', baseUrl: '', postMap: {} });
  ...
  'buildPdfJsScript: uses UMD (not ESM) CDN URLs': () => {
    ...
    assert(script.includes('configurePdfJs(window.pdfjsLib)'), 'preloaded PDF.js should still be configured');
    assert(script.includes('GlobalWorkerOptions.workerSrc'), 'workerSrc should be configured');
    assert(script.includes('encodeURI(el.getAttribute'), 'PDF paths with non-ASCII characters should be encoded');
  ...
  'injectSlideStateRestore: injects refresh-state script before body end': () => {
    const { injectSlideStateRestore } = require('../lib/generator');
    const input = '<html><body><section class="slide active"></section></body></html>';
    const result = injectSlideStateRestore(input);
    assert(result.includes('data-slide-state-restore="true"'), 'state restore marker injected');
    assert(result.indexOf('data-slide-state-restore="true"') < result.indexOf('</body>'), 'script before body end');
    assert(result.includes("slide-current:"), 'uses per-path storage key');
  },

  'injectSlideStateRestore: does not duplicate injected script': () => {
    const { injectSlideStateRestore } = require('../lib/generator');
    const input = '<html><body><section class="slide"></section><script data-slide-state-restore="true"></script></body></html>';
    const result = injectSlideStateRestore(input);
    assert.equal((result.match(/data-slide-state-restore="true"/g) || []).length, 1, 'script injected once');
  },

  'injectSlideStateRestore: ignores non-slide pages': () => {
    const { injectSlideStateRestore } = require('../lib/generator');
    const input = '<html><body><main>No slides</main></body></html>';
    const result = injectSlideStateRestore(input);
    assert.equal(result, input, 'non-slide output unchanged');
  },
};
